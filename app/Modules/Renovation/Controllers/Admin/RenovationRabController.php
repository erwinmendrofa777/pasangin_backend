...
class RenovationRabController extends BaseController
{
    use ResponseTrait;

    protected $renovationRabsModel;
    protected $db;

    public function __construct()
    {
        $this->renovationRabsModel = new RenovationRabsModel();
    ...
    public function save_rab_row()
    {
        ...
        if (!empty($id) && $id != "0") {
            $existing = $this->db->table('renovation_rabs')->where('id', $id)->get()->getRowArray();
    ...
    public function delete_rab_row($id)
    {
        try {
            ...
            if ($this->db->table('renovation_rabs')->where('id', $id)->delete()) {
    ...
    public function add_rab_material()
    {
        ...
        // Cek lock  
        $existing = $this->db->table('renovation_rabs')->where('id', $rabId)->get()->getRowArray();
        if ($existing && $existing['is_locked'] == 1) {
            return $this->response->setJSON(['status' => false, 'message' => 'Tidak bisa tambah, RAB terkunci!']);
        }

        try {
            $this->db->table('renovation_rab_materials')->insert([
                'rab_id' => $rabId,
     ...
     */
    public function save_all_rab($renovationId)
    {
        ...
        if ($shouldLock) {
            // Lock all rows for this renovation
            $this->db->table('renovation_rabs')
                ->where('renovation_id', $renovationId)
                ->update(['is_locked' => 1]);

            // Hitung ulang total dari DB setelah semua row tersimpan
            // agar rows yang sudah ada sebelumnya juga ikut terhitung
            $rabRow = $this->db->query(
                "SELECT COALESCE(SUM(total_price), 0) as rab_sum FROM renovation_rabs WHERE renovation_id = ?",
                [(int) $renovationId]
            )->getRowArray();
    ...
    /**
     * 8. EXPORT RAB TERKUNCI KE EXCEL SESUAI LAYOUT (RENOVASI)
     */
    public function export_rab_excel($renovationId)
    {
        // 1. Ambil data proyek
        ...
        $pekerjaan = 'renovation#' . $project['id'];
        ...
        $sheet->setCellValue('E3', 'Luas Bangunan : -');
        ...
        $sheet->mergeCells('A' . $currentRow . ':E' . $currentRow);
        $sheet->setCellValue('F' . $currentRow, '-'); // No building_area in renovation request, so output hyphen
        ...
        $sheet->getRowDimension($currentRow)->setRowHeight(20);
        $sheet->setCellValue('A' . $currentRow, 'Pembayaran di transfer ke Rekening :');
        ...
        $currentRow++;
        $sheet->getRowDimension($currentRow)->setRowHeight(20);
        $sheet->setCellValue('A' . $currentRow, 'BCA a.n PENDOWO TIGA CONSTRUCTION (PT)');
        $sheet->mergeCells('A' . $currentRow . ':D' . $currentRow);
        $sheet->getStyle('A' . $currentRow)->getFont()->setBold(true);
        ...
        $currentRow++;
        $sheet->getRowDimension($currentRow)->setRowHeight(20);
        $sheet->setCellValue('A' . $currentRow, '814.462.044');
        $sheet->mergeCells('A' . $currentRow . ':D' . $currentRow);
        $sheet->getStyle('A' . $currentRow)->getFont()->setBold(true);
        ...
        header('Content-Disposition: attachment; filename="RAB_Renovasi_' . $project['id'] . '.xlsx"');
    ...
    /**
     * 9. DOWNLOAD TEMPLATE EXCEL KOSONG (RENOVASI)
     */
    public function download_rab_template($renovationId = null)
    {
        $pekerjaan = 'renovation#';
        $lokasi = '-';
        $pemilik = '-';
        ...
        // ── 4. KOSONGKAN DATA ROWS (BARIS 8 - 150) ──
        $startRow = 8;
        $maxDataRows = 150;
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Template_RAB_Renovasi.xlsx"');
        header('Cache-Control: max-age=0');
    ...
    /**
     * 10. IMPORT EXCEL RAB (RENOVASI)
     */
    public function import_rab_excel($renovationId)
    {
        $isLocked = $this->db->table('renovation_rabs')
            ->where('renovation_id', $renovationId)
            ->where('is_locked', 1)
            ->countAllResults() > 0;
        if ($isLocked) {
        ...
        try {
            ...
            $this->db->table('renovation_rabs')->insertBatch($dataToInsert);
