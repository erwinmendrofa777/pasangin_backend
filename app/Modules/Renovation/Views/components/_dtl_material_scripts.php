<script>
  ...
  const demo = <?= json_encode(array_map(function ($sub) {
    ...
    return [
      ...
      'title' => $sub['title'] ?? '',
      'comment' => $sub['comment'] ?? '',
      'photo_url' => !empty($sub['photo']) ? base_url('uploads/renovation/material_submissions/' . $sub['photo']) : null,
    ...
    if (typeof Swal !== 'undefined') {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const actionUrl = "<?= base_url('admin/renovation/update-material-status') ?>/" + id;
  ...
  function selectCustomOption(value, labelText) {
    ...
    if (value) {
      ...
      if (selectedOpt) {
        ...
        selectedOpt.style.color = '#1e40af';
      }
    }
  ...
  function openAddModal() {
    ...
    document.getElementById('formMaterialSubmission').action = '<?= base_url("admin/renovation/add-material-submission") ?>';
  ...
  function openEditModal(id) {
    ...
    document.getElementById('formMaterialSubmission').action = '<?= base_url("admin/renovation/update-material-submission") ?>/' + id;
  ...
  function deleteSubmission(id) {
    ...
    if (typeof Swal !== 'undefined') {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location.href = '<?= base_url("admin/renovation/delete-material-submission") ?>/' + id;
        }
      });
    } else {
