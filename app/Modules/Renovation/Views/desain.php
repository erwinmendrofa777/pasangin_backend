        ...
        <div class="desain-upload-card p-4">
            <div class="d-flex align-items-center mb-3">
                ...
                <div>
                    ...
                    <small class="text-muted">Format: PDF, Gambar, Video — maks. 50 MB</small>
                </div>
            </div>
            <hr class="mt-2 mb-3">
            ...
            <form action="<?= base_url('admin/renovation/add-design/' . $renovation['id']) ?>" method="post"
                ...
                <div class="mb-4">
                    <label class="desain-label">File Desain (PDF, Gambar, Video)</label>
                    <div class="position-relative">
                        <input type="file" name="file_url" class="form-control desain-input"
                            accept=".png,.jpg,.jpeg,.webp,.pdf,.mp4,.mov,.avi,.webm,.mkv" required
            ...
            <div class="row g-3">
                <?php foreach ($design_list as $d):
                    ...
                    $fileUrl = !empty($d['design_requests_id']) 
                        ? base_url('uploads/design_results/' . $d['file_url']) 
                        : base_url('uploads/designs/' . $d['file_url']);
                    ?>
                    <div class="col-6 col-md-4 col-xl-4">
                        <div class="desain-gallery-card">
                            ...
                            <!-- Thumbnail / PDF / Video -->
                            <?php if ($isPdf): ?>
                                <div class="desain-pdf-placeholder" style="cursor:pointer;"
                                    onclick="window.open('<?= $fileUrl ?>', '_blank');">
                                    <i class="fas fa-file-pdf text-danger" style="font-size:2.8rem;"></i>
                                </div>
                            <?php elseif ($isVideo): ?>
                                <div class="desain-pdf-placeholder position-relative" style="background:#fff9f0; cursor:pointer;"
                                    onclick="$('#glb-renovation-video-<?= $d['id'] ?>').click();">
                            ...
                            <?php else: ?>
                                <img src="<?= $fileUrl ?>" class="desain-thumb" alt="<?= esc($d['title'] ?? '') ?>"
                                    style="cursor:pointer;" onclick="$('#glb-renovation-img-<?= $d['id'] ?>').click();">
                            <?php endif; ?>
                            ...
                            <div class="desain-overlay">
                                ...
                                <?php elseif ($isImage): ?>
                                    <a href="<?= $fileUrl ?>" id="glb-renovation-img-<?= $d['id'] ?>"
                                        class="glightbox desain-overlay-btn" data-gallery="design-gallery"
                                        data-title="<?= esc($d['title']) ?>"
                                        data-description="Diunggah oleh: <?= esc($d['admin_name'] ?? 'Sistem') ?> &lt;br&gt; Tanggal: <?= date('d M Y, H:i', strtotime($d['created_at'])) ?> WIB"
                                        title="Lihat Gambar">
                                    ...
                                    <div style="display:none;" id="video-design-<?= $d['id'] ?>">
                                        <div class="p-3 text-center"
                                            ...
                                            <div class="text-white mt-2 text-start px-2">
                                                <h6 class="mb-1 fw-bold text-white"><?= esc($d['title']) ?></h6>
                                                <small class="text-muted">Diunggah oleh: <?= esc($d['admin_name'] ?? 'Sistem') ?> |
                                                    <?= date('d M Y, H:i', strtotime($d['created_at'])) ?> WIB</small>
                                            </div>
                                        </div>
                                    </div>
                                    <a href="#video-design-<?= $d['id'] ?>" class="glightbox desain-overlay-btn"
                                        id="glb-renovation-video-<?= $d['id'] ?>" data-gallery="design-gallery" data-type="inline"
                                        data-slide-class="glightbox-video-slide">
                                        <i class="fas fa-play text-warning"></i>
                                    </a>
                                <?php else: ?>
                                    <a href="<?= $fileUrl ?>" target="_blank" class="desain-overlay-btn" title="Download File">
                            ...
                            <div class="desain-mobile-actions">
                                ...
                                <?php elseif ($isImage): ?>
                                    <button class="btn btn-sm btn-outline-info flex-grow-1"
                                        style="border-radius:8px; font-size:0.78rem;"
                                        onclick="$('#glb-renovation-img-<?= $d['id'] ?>').click();">
