<?php
// FILE: app/Modules/Chat/Views/monitoring.php
// Halaman Chat Monitoring — Hanya percakapan pemantauan klien ↔ supplier

echo $this->extend('layout/app');

echo $this->section('title');
?>
Monitoring Chat - Pasangin
<?php
...
<style>
    ...
    .chat-section-wrapper .btn-outline-primary:hover { background-color: var(--palette-primary) !important; color: #ffffff !important; }
    .chat-card-container { border-radius: 20px !important; overflow: hidden; box-shadow: 0 20px 50px rgba(16,185,129,0.05), 0 5px 15px rgba(0,0,0,0.01) !important; border: 1px solid rgba(16,185,129,0.12) !important; background: #fff; }
    ...
    .search-input-wrapper:focus-within { background: #fff !important; border-color: #10b981 !important; box-shadow: 0 4px 12px rgba(16,185,129,0.12) !important; }
    ...
    .chat-list-user:hover { background-color: #f0fdf4; }
    .chat-list-user.active { background-color: rgba(16,185,129,0.06) !important; border-left-color: #10b981; }
    .chat-list-user.active .chat-client-name { color: #10b981 !important; }
    ...
    .chat-media-preview-container:hover .play-button-overlay i { transform: scale(1.1); background: #10b981; }
    ...
    .pasangin-chat-item.pasangin-chat-right .pasangin-chat-text { background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: #fff; border-radius: 16px 16px 4px 16px; box-shadow: 0 4px 12px rgba(16,185,129,0.2); }
    ...
    .input-wrapper:focus-within { background: #fff; border-color: #10b981; box-shadow: 0 4px 16px rgba(16,185,129,0.1); }
    ...
    .btn-send { width: 44px; height: 44px; border-radius: 50% !important; display: flex; align-items: center; justify-content: center; transition: all 0.3s ease; background: linear-gradient(135deg, #10b981 0%, #059669 100%); border: none; color: #fff; box-shadow: 0 4px 10px rgba(16,185,129,0.3); }
    .btn-send:hover { transform: scale(1.05); box-shadow: 0 6px 14px rgba(16,185,129,0.4); background: linear-gradient(135deg, #10b981 0%, #059669 100%) !important; color: #fff !important; }
    .btn-send:focus, .btn-send:active { outline: none !important; background: linear-gradient(135deg, #10b981 0%, #059669 100%) !important; color: #fff !important; }
    ...
    .pasangin-chat-content::-webkit-scrollbar { width: 5px; } .pasangin-chat-content::-webkit-scrollbar-thumb { background: rgba(16,185,129,0.4); border-radius: 10px; }
    ...
    #attachment-btn:hover { background: #e2e8f0; color: #10b981; }
    ...
    .chat-section-wrapper .section-header { border-radius: 20px !important; margin: 0 !important; border: 1px solid rgba(16,185,129,0.12) !important; box-shadow: 0 20px 50px rgba(16,185,129,0.05), 0 5px 15px rgba(0,0,0,0.01) !important; background: #fff; padding: 16px 25px !important; }
    ...
    #project-info-panel { border-left: 4px solid #6366f1 !important; background: linear-gradient(to right, rgba(99,102,241,0.03), #ffffff) !important; }
    #btn-view-project:hover { background-color: #6366f1 !important; color: #fff !important; }
    /* ========== MONITORING PAGE SPECIFIC ========== */
    .page-type-badge-monitoring {
    ...
    /* Override btn-outline-primary untuk monitoring theme */
    .chat-section-wrapper .btn-outline-primary { color: #10b981 !important; border-color: #10b981 !important; }
    .chat-section-wrapper .btn-outline-primary:hover { background-color: #10b981 !important; color: #ffffff !important; border-color: #10b981 !important; }
    ...
    <div class="section-header d-flex justify-content-between align-items-center py-3 mb-2">
        <div>
            <div class="d-flex align-items-center gap-2 mb-1">
                <span class="page-type-badge-monitoring"><i class="fas fa-eye me-1"></i>Monitoring</span>
            </div>
            <h1 style="font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 1.5rem; color: #1e293b; margin: 0; line-height: 1.2;">Chat Monitoring</h1>
            <div class="text-muted mt-1" style="font-size: 0.8rem; font-weight: 500; letter-spacing: 0.2px;">Pantau percakapan klien dengan supplier secara langsung</div>
        </div>
                ...
                <div class="row g-0">
                    ...
                    <div class="col-md-4 col-12 chat-list-container">
                        <div class="chat-list-header d-flex justify-content-between align-items-center">
                            <h6 class="m-0 fw-bold text-dark" style="font-size: 1rem;"><i class="fas fa-eye me-2" style="color:#10b981;"></i>Monitoring</h6>
                            <div class="d-flex align-items-center" style="gap: 5px;">
                                ...
                                <span class="badge px-2 py-1"
                                    style="font-size: 0.72rem; border-radius: 12px; background-color: rgba(16,185,129,0.08); color: #10b981; border: 1px solid rgba(16,185,129,0.2); font-weight: 600;">
                        ...
                        <div class="chat-search-filter-wrapper">
                            <div class="search-input-wrapper">
                                ...
                                <input type="text" id="chat-search" placeholder="Cari klien atau supplier..." autocomplete="off">
                            ...
                            <ul class="list-unstyled mb-0">
                                <?php if (!empty($conversations)): ?>
                                    <?php foreach ($conversations as $convo): ?>
                                        ...
                                        $avatarUrl = !empty($convo['client_avatar'])
                                            ? (strpos($convo['client_avatar'], 'http') === 0 ? $convo['client_avatar'] : base_url('uploads/profile/' . $convo['client_avatar']))
                                            : "https://ui-avatars.com/api/?name=" . urlencode($convo['client_name'] ?? 'User') . "&background=10b981&color=fff";
                                        ?>
                                        <li class="d-flex chat-list-user p-3 align-items-center border-bottom"
                                            ...
                                            data-category="<?= esc($convo['category'] ?? 'general') ?>"
                                            ...
                                            <div class="flex-grow-1" style="overflow: hidden;">
                                                <div class="d-flex justify-content-between align-items-center mb-1">
                                                    <h6 class="mt-0 mb-0 fw-bold text-truncate text-dark chat-client-name" style="font-size: 0.88rem; max-width: 65%;">
                                                ...
                                                <div class="d-flex justify-content-between align-items-center">
                                                    ...
                                                    <div class="d-flex align-items-center" style="gap: 4px;">
                                                        ...
                                                        <span class="badge px-2 py-1 text-uppercase" style="font-size: 0.58rem; letter-spacing: 0.5px; border-radius: 4px; background-color: #d1fae5; color: #065f46; border: 1px solid #a7f3d0;">Monitored</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </li>
                                ...
                                <?php else: ?>
                                    <li class="text-center p-5 text-muted">
                                        <i class="fas fa-eye fa-2x mb-3 text-light"></i>
                                        <p class="mb-0">Belum ada percakapan monitoring.</p>
                                    </li>
                                <?php endif; ?>
                            </ul>
                        </div>
