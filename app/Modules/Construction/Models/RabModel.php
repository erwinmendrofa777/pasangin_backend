<?php

namespace App\Modules\Construction\Models;

use CodeIgniter\Model;

class RabModel extends Model
{
    // Nama tabel sesuai dengan struktur database terbaru  
    protected $table = 'construction_rabs';
    protected $primaryKey = 'id';
    protected $useAutoIncrement = true;
    protected $returnType = 'array';
    protected $protectFields = true;

    /**
     * allowedFields harus mencakup semua kolom yang kita gunakan di Controller
     * Termasuk kolom is_locked untuk fitur kunci RAB  
     */
    protected $allowedFields = [
        'construction_id',
        'roman_number',
        'group_name',
        'sub_group_name',
        'section_group',
        'section_name',
        'ahsp_id',
        'volume',
        'unit',
        'selected_material_id',
        'current_unit_price',
        'total_price',
        'is_locked' // Tambahkan ini agar fitur Lock RAB berfungsi  
    ];

    // Aktifkan timestamps sesuai kolom yang kita tambahkan di SQL tadi
    protected $useTimestamps = true;
    protected $createdField = 'created_at';
    protected $updatedField = 'updated_at';

    /**
     * Fungsi helper untuk mengambil data RAB per proyek
     * Diurutkan berdasarkan Roman dan ID agar sinkron dengan Flutter  
     */
    public function getRabWithMaterials($construction_id)
    {
        return $this->where('construction_id', $construction_id)
            ->orderBy('roman_number', 'ASC')
            ->orderBy('id', 'ASC')
            ->findAll();
    }

    /**
     * Fungsi opsional untuk mengecek status lock sebuah baris  
     */
    public function isLocked($id)
    {
        $row = $this->select('is_locked')->find($id);
        return $row ? (bool) $row['is_locked'] : false;
    }
}