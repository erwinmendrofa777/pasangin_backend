<script>
    ...
    function loadRabMaterials(rabId) {
        ...
        $.get('<?= base_url('admin/construction/get_rab_materials') ?>/' + rabId, function (data) {
            ...
            $(document).off('click', '.select-product-radio').on('click', '.select-product-radio', function () {
                ...
                $.post('<?= base_url('admin/construction/select_rab_material') ?>', {
                    '<?= csrf_token() ?>': '<?= csrf_hash() ?>',
                    id: recId
                }, function (res) {
            ...
            $(document).off('click', '.delete-rec-btn').on('click', '.delete-rec-btn', function () {
                ...
                $.get('<?= base_url('admin/construction/delete_rab_material') ?>/' + recId, function (res) {
                    container.css('opacity', '1');
                    if (res.status) {
                        loadRabMaterials(activeRabId);
    ...
    function recalculateAhspRabPrice(rabId) {
        ...
        $.get('<?= base_url('admin/construction/recalculate_rab_price') ?>/' + rabId, function (res) {
            if (res.status) {
