...
<div class="modal fade modal-rab" id="modalRabMaterials" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
        <div class="modal-content" style="border-radius: 16px; border: none; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
            <div class="modal-header" style="background: var(--palette-primary); color: white; border-top-left-radius: 16px; border-top-right-radius: 16px; padding: 16px 20px;">
                <h5 class="modal-title text-white" id="modalRabMaterialTitle"><i class="fas fa-boxes me-2"></i> Pilihan Produk untuk Bahan Pekerjaan</h5>
            ...
            <div class="modal-body" style="max-height: 600px; overflow-y: auto; padding: 20px;">
                <div class="alert alert-info d-flex align-items-start gap-2 mb-4" style="font-size: 13px;">
                    ...
                    <div>
                        <strong>Petunjuk:</strong> Pekerjaan ini memiliki beberapa kebutuhan bahan/material. Silakan tentukan produk spesifik yang akan digunakan untuk masing-masing kebutuhan bahan di bawah ini. Harga satuan pekerjaan akan dihitung ulang secara otomatis setelah Anda memilih produk.
                    </div>
                </div>
...
<div class="modal fade modal-rab" id="modalAhspDetail" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
        <div class="modal-content" style="border-radius: 16px; border: none; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
            <div class="modal-header" style="background: var(--palette-primary); color: white; border-top-left-radius: 16px; border-top-right-radius: 16px; padding: 16px 20px;">
                <h5 class="modal-title text-white" id="modalAhspDetailTitle"><i class="fas fa-clipboard-list me-2"></i> Detail AHSP</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Tutup"></button>
            </div>
            <div class="modal-body" id="modalAhspDetailBody" style="max-height: 70vh; overflow-y: auto; padding: 24px; background: #f8fafc;">
                <!-- Populated dynamically via JS -->
        ...
        <div class="modal-content" style="border-radius: 16px; border: none; box-shadow: 0 10px 30px rgba(0,0,0,0.15);">
            ...
            <div class="modal-body" style="max-height: 550px; overflow-y: auto; padding: 20px;">
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle" id="tableAhspPicker"
                        ...
                        <tbody>
                            <?php if (!empty($ahsp_list)): ?>
                                <?php foreach ($ahsp_list as $a): ?>
                                    <tr class="ahsp-picker-row" data-id="<?= $a['id'] ?>"
                                        data-kode="<?= esc($a['kode']) ?>" data-uraian="<?= esc($a['uraian']) ?>">
                                        <td class="text-center toggle-details" style="padding: 12px; color: #64748b; font-size: 14px; cursor: pointer;">
                                            <i class="fas fa-chevron-right toggle-icon" style="transition: transform 0.25s ease;"></i>
                                        </td>
                                        <td class="fw-bold text-primary select-row-action" style="padding: 12px; cursor: pointer;"><?= esc($a['kode']) ?></td>
                                        <td class="fw-bold text-dark uraian-cell select-row-action" style="padding: 12px; cursor: pointer;">
                                            <?= esc($a['uraian']) ?></td>
                                    ...
                                    <tr class="ahsp-detail-row" id="detail-ahsp-<?= $a['id'] ?>" style="display: none; background: #f8fafc;">
                                        <td></td>
                                            ...
                                            <div class="accordion" id="accordionAhsp-<?= $a['id'] ?>">
                                                ...
                                                <div class="accordion-item" style="border-radius: 10px; border: 1px solid #e2e8f0; margin-bottom: 8px; overflow: hidden; background: #fff;">
                                                    <h2 class="accordion-header" id="headingBahan-<?= $a['id'] ?>">
                                                        <button class="accordion-button collapsed py-2 px-3 fw-bold" type="button" data-bs-toggle="collapse" data-bs-target="#collapseBahan-<?= $a['id'] ?>" aria-expanded="false" aria-controls="collapseBahan-<?= $a['id'] ?>" style="font-size: 11px; color: #475569; background: #f8fafc; border: none; box-shadow: none;">
                                                            <i class="fas fa-boxes me-2 text-primary"></i> Rincian Bahan / Material (<?= count($a['bahan'] ?? []) ?>)
                                                        </button>
                                                    </h2>
                                                    <div id="collapseBahan-<?= $a['id'] ?>" class="accordion-collapse collapse" aria-labelledby="headingBahan-<?= $a['id'] ?>" data-bs-parent="#accordionAhsp-<?= $a['id'] ?>">
                                                        <div class="accordion-body p-2" style="background: #fff;">
                                                            <?php if (!empty($a['bahan'])): ?>
                                                                <table class="table table-bordered table-sm m-0" style="font-size: 10px;">
                                                                    ...
                                                                    <tbody>
                                                                        <?php $noB = 1; foreach ($a['bahan'] as $b): ?>
                                                                            <tr>
                                                ...
                                                <div class="accordion-item" style="border-radius: 10px; border: 1px solid #e2e8f0; overflow: hidden; background: #fff;">
                                                    <h2 class="accordion-header" id="headingTenaga-<?= $a['id'] ?>">
                                                        <button class="accordion-button collapsed py-2 px-3 fw-bold" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTenaga-<?= $a['id'] ?>" aria-expanded="false" aria-controls="collapseTenaga-<?= $a['id'] ?>" style="font-size: 11px; color: #475569; background: #f8fafc; border: none; box-shadow: none;">
                                                            <i class="fas fa-users me-2 text-primary"></i> Rincian Tenaga Kerja (<?= count($a['tenaga_kerja'] ?? []) ?>)
                                                        </button>
                                                    </h2>
                                                    <div id="collapseTenaga-<?= $a['id'] ?>" class="accordion-collapse collapse" aria-labelledby="headingTenaga-<?= $a['id'] ?>" data-bs-parent="#accordionAhsp-<?= $a['id'] ?>">
                                                        <div class="accordion-body p-2" style="background: #fff;">
                                                            <?php if (!empty($a['tenaga_kerja'])): ?>
                                                                <table class="table table-bordered table-sm m-0" style="font-size: 10px;">
                                                                    ...
                                                                    <tbody>
                                                                        <?php $noT = 1; foreach ($a['tenaga_kerja'] as $t): 
                                                                        ...
                                                                        ?>
                                                                            <tr>
                                                                                ...
                                                                                <td class="text-end font-monospace fw-bold text-dark">Rp <?= number_format($subTotalT, 0, ',', '.') ?></td>
                                                                            </tr>
                                                                        <?php endforeach; ?>
                                                                    </tbody>
                                                                </table>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="4" class="text-center text-muted" style="padding: 20px;">Tidak ada data AHSP.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                ...
                <div class="d-flex justify-content-between align-items-center mt-3 pt-3" style="border-top: 1px solid #f1f5f9;">
                    <div class="text-muted" id="paginationInfo" style="font-size: 12px; font-weight: 500;">Menampilkan 0 - 0 dari 0 data</div>
