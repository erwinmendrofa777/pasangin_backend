<style>
    ...
    .table-rab th:nth-child(1),
    .input-roman {
        min-width: 60px;
    }

    .table-rab th:nth-child(2),
    .input-group-name {
        min-width: 160px;
    }

    .table-rab th:nth-child(3),
    .input-section {
        min-width: 160px;
    }

    .table-rab th:nth-child(4),
    .input-task {
        min-width: 220px;
    }

    .table-rab th:nth-child(5),
    .input-vol {
        min-width: 70px;
    }

    .table-rab th:nth-child(6),
    .input-unit {
        min-width: 80px;
    }

    .table-rab th:nth-child(7),
    .input-price {
        min-width: 130px;
    }

    .table-rab th:nth-child(8),
    .row-total {
        min-width: 130px;
    }

    .table-rab th:nth-child(9) {
        min-width: 110px;
    }
...
<div class="d-flex justify-content-between align-items-center mb-3 rab-header-actions">
    <h6 class="font-weight-bold text-primary mb-0"><i class="fas fa-file-invoice-dollar mr-2"></i> Manajemen RAB Proyek
    </h6>
    <div class="d-flex gap-2 flex-wrap">
        <?php if (!empty($rab_list) && $rab_list[0]['is_locked'] == 0): ?>
            <a href="<?= base_url('admin/renovation/lock_rab/' . $renovation['id']) ?>"
                class="btn btn-danger btn-sm shadow-sm flex-grow-1"
                onclick="return confirm('Kunci RAB kawan? Data tidak bisa diubah lagi!')"><i class="fas fa-lock mt-1"></i>
                Kunci RAB</a>
        <?php elseif (!empty($rab_list)): ?>
            <a href="<?= base_url('admin/renovation/unlock_rab/' . $renovation['id']) ?>"
                class="btn btn-warning btn-sm shadow-sm flex-grow-1"><i class="fas fa-lock-open mt-1"></i> Buka Kunci</a>
        <?php endif; ?>
        <a href="<?= base_url('admin/renovation/cetak-pdf/' . $renovation['id']) ?>" target="_blank" class="btn btn-info btn-sm shadow-sm flex-grow-1"><i class="fas fa-file-pdf mt-1"></i> Preview Kontrak</a>
        <button class="btn btn-success btn-sm shadow-sm flex-grow-1" onclick="addNewRabRow()"><i
                class="fas fa-plus mt-1"></i> Tambah Baris</button>
    </div>
</div>
            ...
            <?php if (!empty($rab_list)):
                foreach ($rab_list as $rab): ?>
                    <tr data-id="<?= $rab['id'] ?>" class="<?= $rab['is_locked'] ? 'row-locked' : '' ?>">
                        ...
                        <td><input type="text" class="input-group-name" value="<?= esc($rab['group_name'] ?? 'PEKERJAAN') ?>">
                        </td>
                        ...
                        <td><input type="number" class="input-price text-right" value="<?= (int) $rab['current_unit_price'] ?>">
                        </td>
                            ...
                            <?php else: ?>
                                <div class="btn-group">
                                    <button class="btn btn-primary btn-xs"
                                        onclick="openMaterialModal(<?= $rab['id'] ?>, '<?= esc($rab['activity_name']) ?>')"><i
                                            class="fas fa-boxes"></i></button>
                                    <button class="btn btn-success btn-xs" onclick="saveRabRow(this)"><i
                                            class="fas fa-save"></i></button>
                                    <button class="btn btn-danger btn-xs" onclick="deleteRabRow(this, <?= $rab['id'] ?>)"><i
...
<script>
    ...
    function deleteMaterial(id) {
        if (confirm('Hapus bahan?')) {
            $.get('<?= base_url('admin/renovation/delete_rab_material') ?>/' + id, function () {
                openMaterialModal(activeRabId, '');
            });
        }
    }
