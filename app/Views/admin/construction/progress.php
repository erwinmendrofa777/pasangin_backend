<style>
    ...
    .target-group-card:nth-child(1) { animation-delay: 0.05s; }
    .target-group-card:nth-child(2) { animation-delay: 0.10s; }
    .target-group-card:nth-child(3) { animation-delay: 0.15s; }
    ...
    .target-group-header:hover { background: #eaedff; }
    ...
    .progress-item-card:hover::before { background: #6777ef; }
    .progress-item-card.st-approved::before { background: #47c363; }
    .progress-item-card.st-rejected::before { background: #fc544b; }
    .progress-item-card.st-pending::before  { background: #ffa426; }
    ...
    .prog-status-pill:hover { filter: brightness(0.92); }
    .pill-approved { background: #d1e7dd; color: #0a5c36; }
    .pill-rejected { background: #f8d7da; color: #842029; }
    .pill-pending  { background: #fff3cd; color: #7d5a00; }
    ...
    .prog-dropdown .dropdown-item:hover { background: #f0f3ff; }
    ...
    @media (max-width: 767px) {
        ...
        .progress-item-card > .d-flex > :nth-child(1) { grid-area: num; }
        .progress-item-card > .d-flex > :nth-child(2) { grid-area: photo; }
        .progress-item-card > .d-flex > :nth-child(3) { grid-area: content; }
        .progress-item-card > .d-flex > :nth-child(4) { grid-area: dropdown; }
...
<div class="progress-header">
    ...
    <div class="d-flex align-items-center gap-2">
        ...
        <span class="progress-count-badge" style="background: linear-gradient(135deg, #47c363, #5ad178);"><?= $totalProgress ?> Laporan</span>
    </div>
</div>

<?php if (!empty($groupedProgress)): ?>
    <?php $groupIdx = 0; foreach ($groupedProgress as $targetName => $items):
        $groupIdx++;
    ...
    <div class="target-group-card">
            ...
            <div class="target-name">
                ...
                <div>
                    ...
                    <div style="font-size:0.72rem; font-weight:400; color:#6c757d; margin-top:1px;">
                        <i class="fas fa-check-circle text-success mr-1"></i><?= $approvedCount ?>/<?= $totalItems ?> disetujui
                    </div>
                </div>
            </div>
            <div class="tg-meta">
        ...
        <div class="collapse show" id="<?= $collapseId ?>">
            <div class="target-group-body">
                <?php $localNo = 0; foreach ($items as $app):
                    $localNo++;
                    $st = strtoupper($app['status'] ?? 'PENDING');
                    $stClass = 'st-pending';
                    $pillClass = 'pill-pending';
                    if ($st === 'APPROVED') { $stClass = 'st-approved'; $pillClass = 'pill-approved'; }
                    elseif ($st === 'REJECTED') { $stClass = 'st-rejected'; $pillClass = 'pill-rejected'; }
                ?>
                <div class="progress-item-card <?= $stClass ?>">
                    <div class="d-flex align-items-start gap-3">
                        ...
                        <?php if (!empty($app['photo'])): ?>
                            <a href="<?= base_url('uploads/construction/progress/' . $app['photo']) ?>" target="_blank">
                                <img src="<?= base_url('uploads/construction/progress/' . $app['photo']) ?>" class="prog-photo-thumb" alt="Foto Progress">
                            </a>
                        <?php else: ?>
                        ...
                        <div class="flex-shrink-0 prog-dropdown">
                            <div class="dropdown">
                                <button type="button"
                                    class="prog-status-pill <?= $pillClass ?> dropdown-toggle"
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    <li>
                                        <a class="dropdown-item <?= $st === 'PENDING' ? 'active bg-warning text-white' : '' ?>" href="#"
                                           onclick="event.preventDefault(); updateProgressStatus(this, <?= $app['id'] ?>, 'PENDING', '<?= $st ?>')">
                                            <i class="fas fa-clock mr-2 text-warning"></i>PENDING
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item <?= $st === 'APPROVED' ? 'active bg-success text-white' : '' ?>" href="#"
                                    ...
                                    <li>
                                        <a class="dropdown-item <?= $st === 'REJECTED' ? 'active bg-danger text-white' : '' ?>" href="#"
...
<script>
    function updateProgressStatus(el, id, newStatus, oldStatus) {
        ...
        $.post('<?= base_url('admin/construction/update_progress_status') ?>/' + id + '/' + newStatus, {
