                                    ...
                                    <td class="text-center">
                                        <?php if ($supplier['status'] === 'pending') : ?>
                                            ...
                                            <form action="<?= route_to('admin.supplier.update_status', $supplier['id'], 'approved') ?>" method="post" class="d-inline">
                                                <?= csrf_field() ?>
                                                <button type="submit" class="btn btn-icon icon-left btn-success btn-sm ladda-button" data-style="zoom-in" onclick="return confirm('Anda yakin ingin MENYETUJUI supplier ini?')">
                                            ...
                                            <form action="<?= route_to('admin.supplier.update_status', $supplier['id'], 'rejected') ?>" method="post" class="d-inline">
                                                <?= csrf_field() ?>
                                                <button type="submit" class="btn btn-icon icon-left btn-danger btn-sm ladda-button" data-style="zoom-in" onclick="return confirm('Anda yakin ingin MENOLAK supplier ini?')">
                                        ...
                                        <?php elseif ($supplier['status'] === 'approved') : ?>
                                            ...
                                            <form action="<?= route_to('admin.supplier.update_status', $supplier['id'], 'banned') ?>" method="post" class="d-inline">
                                                <?= csrf_field() ?>
                                                <button type="submit" class="btn btn-icon icon-left btn-danger btn-sm ladda-button" data-style="zoom-in" onclick="return confirm('Anda yakin ingin MEMBLOKIR supplier ini?')">
                                        ...
                                        <?php else : // Untuk status 'rejected' atau 'banned' ?>
                                            ...
                                            <form action="<?= route_to('admin.supplier.update_status', $supplier['id'], 'approved') ?>" method="post" class="d-inline">
                                                <?= csrf_field() ?>
                                                <button type="submit" class="btn btn-icon icon-left btn-info btn-sm ladda-button" data-style="zoom-in" onclick="return confirm('Anda yakin ingin MENGAKTIFKAN KEMBALI supplier ini?')">
...
$(document).ready(function() {
    ...
    var table = $('#table-1').DataTable({
        ...
        "columnDefs": [
            { "sortable": false, "targets": [2,3,4,6] } // Foto dan Aksi tidak bisa di-sort
        ],
        "pageLength": 10,
        "searching": true,
        "ordering": true,
